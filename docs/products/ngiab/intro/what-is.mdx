---
sidebar_position: 1
title: "What is the NextGen Framework?"
...
is a hydrologic modeling framework currently under development for use in the National Water Model.
The National Water Model is the United States' core water prediction framework, offering predictions
for the Continental United States (CONUS), Alaska, Hawaii, Puerto Rico, and the U.S. Virgin Islands.

Prior to NextGen, the National Water Model applied the same module formulation everywhere, which didn't
...
that improves regional interoperability and opening model contributions to the broader hydrological community.
...
[NGen](https://github.com/NOAA-OWP/ngen/tree/master) is the canonical implementation of the NextGen framework,
responsible for administering connections between encapsulated models.
While NGen is currently still under development, it's already become an incredibly powerful tool for novel approaches to hydrology.
...
This can already be time-consuming a time-consuming task for experienced programmers,
so for scientists wanting to apply it in their own research,
installing and configuring NGen is extremely unapproachable.
...
If you'd like to get started with NextGen In A Box, check out the NGIAB 101 module,
